...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if self.exit != None:
            self.exit.entrance = self # If a Place has an exit, then the exit's entrance is set to that Place
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        """
        if insect.is_ant: #If the ant is an insect
            if self.ant is None: #If there is an ant
                self.ant = insect #Update instance variable 'ant' with our ant
            else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt and insect.true_queen == True:
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    watersafe = True # Part of Problem 3
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 11
        if hasattr(self, 'ant'):
            if (self.container) & (self.ant == None) & (other.container == False):
                return True
        return False
        # END Problem 11
...
class HarvesterAnt(Ant):
    ...
    def action(self, colony):
        ...
        colony.food += 1 # Increases the colonies' food by 1


class ThrowerAnt(Ant):
    """ThrowerAnt throws a leaf each turn at the nearest Bee in its range."""
    
    food_cost = 3 # Part of Problem 1
    name = 'Thrower'
    implemented = True
    damage = 1
    min_range = 0
    ...
    def nearest_bee(self, hive):
        ...
        while True:
            if (place.bees != []) & (counter >= min_range) & (counter <= max_range) and place != hive:
...
class FireAnt(Ant):
    ...
    food_cost = 5 # Part of Problem 4
    armor = 1 # Part of Problem 4
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor > amount:
            ...
            for elem in self.place.bees:
                if elem.armor - self.damage > 0:
                    elem.armor -= self.damage
                    bee_lst += [elem]
...
class WallAnt(Ant):
    ...
    def __init__(self): #Required; otherwise bug exists
        self.name = 'Wall'
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        ##Double Damage   
            while True: 
                if place.exit == None or type(place) == Hive:
                    break
                else:
                    if place.ant != None and place.ant != self:

                        ##For Container Ants
                        if hasattr(place.ant, 'container'):
                            if hasattr(place.ant, 'damage') and place.ant.queen_doubled_dam == None:
                                place.ant.damage += place.ant.damage
                                place.ant.queen_doubled_dam = True
                            if place.ant.container == True:
                                ##Make sure queen ant's damage is not doubled
                                if place.ant.ant == self:
                                    pass
                                else:
                                    if hasattr(place.ant.ant, 'damage') and place.ant.ant.queen_doubled_dam == None:
                                        place.ant.ant.damage +=place.ant.ant.damage
                                        place.ant.ant.queen_doubled_dam = True
                        ##For All Other Ants
                        else:
                                if hasattr(place.ant, 'damage') and place.ant.queen_doubled_dam == None:
                                    place.ant.damage += place.ant.damage
                                    place.ant.queen_doubled_dam = True
                    place = place.exit
                ##Turn off Doubled Damage Ability 
                
        ##If Imposter
        
        if self.true_queen == False:
            self.armor = 0
            ##check for container ant
            if type(self.place.ant) == QueenAnt:
                self.place.ant = None
                self.place = None 
            else: 
